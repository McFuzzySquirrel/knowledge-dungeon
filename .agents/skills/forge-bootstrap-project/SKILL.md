...
- **Preserve all existing outputs.** The artifacts produced (`docs/PRD.md`,
  `.opencode/agents/*.md`, `.agents/skills/*/SKILL.md`, `docs/MODEL-PLAN.md`,
...
- **Resumability.** If the user starts this skill in a repo that already has a
  `docs/PRD.md` or existing agent files in `.opencode/agents/`, detect it and offer to
...
3. **Check repo state** and flag anything that affects the flow:
   ...
   - Do agent files already exist in `.opencode/agents/` (beyond the forge
...
When it finishes and agent files have been written under `.opencode/agents/`
...
- **No new file formats.** Do not introduce a new state file, manifest, or
  ...
  (`docs/PRD.md`, `.opencode/agents/*.md`, `docs/MODEL-PLAN.md`) are the
  state.
...
- **Idempotent re-entry edge case.** If `docs/PRD.md` exists but `.opencode/agents/` has no agent files, resume from Step 2 (team building). If both exist, assume the flow completed and summarize what was produced - don't re-run.
- **Feature Increment mode on re-entry.** If `.opencode/agents/` already has agent files and the user says "bootstrap my project," Step 0's repo check must warn that Step 2 will run in Feature Increment Mode, not Full Build. Confirm with the user or they'll get unexpected behavior.
