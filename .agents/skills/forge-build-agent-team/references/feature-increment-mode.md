...
Load this file when: the source document is a Feature PRD (has "Feature Overview", "Context: Existing System State", "Agent Impact Assessment") and existing `.md` files already exist in `.opencode/agents/`.
...
2. **Read ALL existing agent files** in `.opencode/agents/`:
...
- **For new agents:** Write complete agent files at `.opencode/agents/{agent-name}.md` following the standard template from Step 5.
