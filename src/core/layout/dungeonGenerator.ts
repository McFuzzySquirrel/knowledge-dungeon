 ...
 * dungeonGenerator
 *
 * Renderer-neutral: transforms a Subject's topic graph (DungeonMetadata) into
 * a tile-based dungeon layout: a grid of rooms placed via BFS from the root,
 * connected by axis-aligned (straight or L-shaped) corridors that mirror the
 * parent/child + cross-link edges from the mindmap. Any renderer adapter (the
 * Phaser scene today, the PixiJS host later) renders the resulting DungeonMap.
 *
